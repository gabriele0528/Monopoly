...
const boardBottom = document.querySelector("#board-bottom");
const boardLeft = document.querySelector("#board-left");
const diceButton = document.querySelector("#dice-button");
const diceResult = document.querySelector("#dice-result");
const playerTurn = document.querySelector("#turned-player");
const players = [
    {
        name: "Player 1",
        money: 1000,
...
function getCurrentCard(position) {
    return cards[position - 1];
}
function ownsAllPropertiesOfColor(playerIndex, color) {
    if (!color)
        return false;
    const propertiesOfColor = cards.filter(c => c.color === color);
    return propertiesOfColor.every(prop => { var _a; return ((_a = players[playerIndex].properties) === null || _a === void 0 ? void 0 : _a.indexOf(prop.name)) !== -1; });
}
function canBuyProperty(playerIndex, position) {
...
function canBuyHouse(playerIndex, propertyName) {
...
function canBuyHotel(playerIndex, propertyName) {
    const property = cards.find(c => c.name === propertyName);
    if (!property)
        return false;
    const player = players[playerIndex];
...
function handlePropertyFees(playerIndex, position) {
    const card = getCurrentCard(position);
    const player = players[playerIndex];
...
function updatePlayersDisplay() {
    [".first-player", ".second-player"].forEach((selector, index) => {
        const playerInfo = document.querySelector(selector);
        ...
        playerInfo.innerHTML = `
            ...
            return `
                        <div class="own-card" style="border-top: 3px solid ${(card === null || card === void 0 ? void 0 : card.color) || 'transparent'}">
                            ${prop}
            ...
            <div class="buy-properties d-flex j-between gap-20">
                ...
                <button type="button" class="btn btn-secondary" ${currentPlayerIndex === index && canBuyHouse(index, currentCard === null || currentCard === void 0 ? void 0 : currentCard.name) ? '' : 'disabled'}>Buy House</button>
                <button type="button" class="btn btn-success" ${currentPlayerIndex === index && canBuyHotel(index, currentCard === null || currentCard === void 0 ? void 0 : currentCard.name) ? '' : 'disabled'}>Buy Hotel</button>
            </div>
        `;
    });
...
function attachButtonListeners() {
    document.querySelectorAll('.btn-primary').forEach((btn, index) => {
        btn.onclick = () => currentPlayerIndex === index && buyProperty();
    });
